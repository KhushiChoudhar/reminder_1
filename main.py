...
API_TOKEN = " "       # API Token generated by the BotFather
...
def Send_Welcome(message):
    cursor = mydb.cursor()
    query = "SELECT chatId FROM users WHERE chatId=%s"   # Searches the user's ID if it is already registered or not
    val = (message.chat.id,)
    cursor.execute(query, val)
    cursor.fetchall()
    if cursor.rowcount == 0:      # If the user is using the Bot first time or his/her ID is not found in the databse
        msg = bot.reply_to(message, 'Hello, Welcome to the Bot.\n\n Pls Enter a Username: \n\n/Exit')      # The user is asked to enter a username for him/her
        bot.register_next_step_handler(msg, Register_User)                 # The user's username gets registered in the database
    
    else:                      # If the user is already registered and his/her ID exists in the database
        ...
        # Bot's response to user after he/she is registered
...
def get(message):
    ...
    # Add, View, Delete and Exit options will get displayed for the user to choose
...
def Register_User(message):
    value = message.text
    if value == '/Exit':
        # If exit command is found, then assign itembtn as start, add it and print message of GoodBye to the user
        markup = types.ReplyKeyboardMarkup()
        itembtn = types.KeyboardButton('/Start')
        markup.add(itembtn)
        msg = bot.reply_to(message, 'Well then, Good Bye.',reply_markup=markup)
...
def Add_Reminder(message):
    ...
    if value == '/Exit':
        ...
        # Options of Categories for which the reminder has to be added for the user 
...
# Start details for the reminder
# Add Date and Time details for the reminder
def Add_Category_Reminder_Start_Date(message):
    ...
    if value == '/Exit':
        ...
        # These date options will get displayed on the keyboard for the user to choose
...
def Add_Month(message):
    ...
    if value == '/Exit':
        ...
        # These month options will get displayed for the user
...
def Add_Year(message):
    ...
    if value == '/Exit':
        ...
        # These year options will get displayed on the keyboard for the user to choose
...
def Add_Time(message):
    ...
    if value == '/Exit':
        ...
        # These hour options will get displayed on the keyboard for the user
...
def Add_Minute(message):
    ...
    if (value == '/Exit'):
        ...
        # AM and PM options for the user
        ...
        # These minute options will get displayed on the keyboard for the user
        markup = types.ReplyKeyboardMarkup(row_width=4)
...
def Add_Context(message):
    ...
    if value == '/Exit':
        ...
        # Reply to the user as per the category chosen for the purpose the reminder bot is being used
        if categoryId == 1 :
...
def Add_Category_Reminder_End_Date(message):
    ...
    if value == '/Exit':
        ...
        # Date options that will get displayed on the keyboard for the user
        itembtn1 = types.KeyboardButton('01')
        itembtn2 = types.KeyboardButton('02')
        itembtn3 = types.KeyboardButton('03')
        itembtn4 = types.KeyboardButton('04')
...
# End Month details for the reminder
def Add_End_Month(message):
    ...
    if value == '/Exit':
        ...
        # Month options that will get displayed on the keyboard for the user
        jan = types.KeyboardButton('January')
        feb = types.KeyboardButton('February')
        mar = types.KeyboardButton('March')
        apr = types.KeyboardButton('April')
...
# End year details for the rmeinder
def Add_End_Year(message):
    ...
    if value == '/Exit':
        ...
        # Year options that will get displayed on the keyboard for the user
        y2022 = types.KeyboardButton('2022')
        y2023 = types.KeyboardButton('2023')
        y2024 = types.KeyboardButton('2024')
        y2025 = types.KeyboardButton('2025')
...
# Add Reminder details to the database
def Add_Reminder_To_DB(message):
    ...
    if value == '/Exit':
        ...
        date1 = value + '-' + date1
        if categoryId == 1:        # If the Category of reminder chosen is MEETING
            cursor = mydb.cursor()
            ...
            cursor.execute(query, val)
            mydb.commit()
        elif categoryId == 2:        # If the Category of reminder chosen is TASK
            cursor = mydb.cursor()
            query = ("INSERT INTO reminders(chatId, date, time, message,end_date) VALUES (%s,%s,%s,%s,%s)")
            ...
            cursor.execute(query, val)
            mydb.commit()
        elif categoryId == 3:       # If the Category of reminder chosen is OTHERS
            cursor = mydb.cursor()
            query = ("INSERT INTO reminders(chatId, date, time, message,end_date) VALUES (%s,%s,%s,%s,%s)")
        ...
        # Now the reminder with all the details has been added and stored
...
def View_Reminders(message):
    ...
    if value == '/Exit':
        ...
        cursor = mydb.cursor()
        # The reminders are selected from the database
        query = ("SELECT DISTINCT date FROM reminders WHERE chatId= %s ORDER BY date ASC")
        val = (message.chat.id,)
        cursor.execute(query, val)
        resultset=cursor.fetchall()
        msgstr=''
        if(cursor.rowcount==0):     # If there are no reminders found 
            msg = bot.reply_to(message,"Nothing to View /Start")
            bot.register_next_step_handler(msg, Send_Welcome)
        else:        # The reminders are displayed
            for i in resultset:
                msgstr+='\nDate : '+str(i[0])+'\n'
...
# Handles '/Delete_Reminder' command
...
def Delete_Reminders(message):
    ...
    if value == '/Exit':
        ...
        cursor = mydb.cursor()
        # Select reminder from the database to be deleted
        ...
        if(cursor.rowcount==0):
            # The reminder to be deleted is not present in the database
            msg = bot.reply_to(message,"Nothing to Delete /Start")
            bot.register_next_step_handler(msg, Send_Welcome)
        else:
            # The reminder is found and deleted
            msgstr = '\n'
            count = 1
            for i in resultSet:
